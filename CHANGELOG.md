...
- README 增加**完整演示 GIF**(`docs/images/demo-full.gif`,640×416、10fps、4× 加速、约 4.8MB),
  ...
  (2× 加速 / 1280px / 8fps / **256 色** / bayer:5,约 19.8MB——调色板拉满、抖动降最弱,颜色贴近原片)
...
- **自建 Bark 服务器**的地址不再预填失败:预填判定改为"http(s) 开头即认";
  `normalizedURL` 会去掉复制带入的**换行/空格**(自建域名也支持)

### Fixed
- **同窗口多个 success 一直不灭**:`acknowledgeFocused` 的候选里没排除已确认会话,
  而每次只确认"最新一条",导致时间最新的**已确认**会话被反复选中,其它会话永远轮不到。
  现候选排除 `ack`,并改为**一次切换确认该窗口(同目录)下所有未确认会话**
