...
- **会话管理**:菜单显示上限(5/10/20/50/不限,默认 10,超出折叠为「…还有 N 个」);
  手动清理(清理已确认/过期、只保留最近 50 个、清空全部),清理时一并清掉名字/目录/宿主侧表条目
- **屏蔽会话**:「会话管理 → 屏蔽会话…」把某条会话加入 `~/.ai-status-light/blocked.json`,它会被**彻底忽略**(菜单不显示、不影响灯/气泡/推送/确认),状态文件立即删除;
  桥接层(以及 opencode 插件)对屏蔽会话**不再写入状态**,真正做到"不再监听";「已屏蔽 (N)…」里点击可取消
...
- **success 常驻**:任务完成后绿灯会一直亮,直到**被「唤起」**才不再绿灯(会话**仍保留在菜单**里,置灰):
  ...
  - **直接切到该会话窗口**(菜单「切到窗口即确认」,默认开;精确匹配窗口标题需**辅助功能**权限)
    - 一次切换会确认**该窗口(同目录)下所有未确认**的 完成/出错 会话;已确认的不会重复参与
    - 注意:同目录的两个 VS Code 窗口无法区分(opencode 只上报目录),会一起确认
